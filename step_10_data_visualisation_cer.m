...
main_y = niftiread('main_y_cer.nii');
para_y = niftiread('para_y_cer.nii');
% old only exp x control
main_o = niftiread('main_o_cer.nii');
para_o = niftiread('para_o_cer.nii');
% age x condition interaction
main = niftiread('main_cer.nii');
para = niftiread('para_cer.nii');
...
cd('/MRIWork/MRIWork06/nr/matthew_danvers/Study_3/eye_movements_and_rules/data/2nd_level/covariate_version/images/age_comp_cer');
    set(gca, 'XTick', xt, 'XTickLabel', round(xt/sec,2));


clear n
% work through the ma    set(gca, 'XTick', xt, 'XTickLabel', round(xt/sec,2));
in effects
...
% look at young only
cd('/MRIWork/MRIWork06/nr/matthew_danvers/Study_3/eye_movements_and_rules/data/2nd_level/covariate_version/images/young_cer');
positions.main_y = find(main_y > 0);
